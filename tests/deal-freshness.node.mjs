...
test("month-only parsing uses the correct leap-year month end", () => {
  const label = "Affiliate datafeed price, February 2024. Confirm current price before buying."
...
test("the parser distinguishes exact and month-only catalog labels", () => {
  ...
  assert.deepEqual(freshness.parseDealLastChecked("Affiliate datafeed price, June 2026. Confirm current price."), {
    kind: "month",
    year: 2026,
    month: 5,
  })
