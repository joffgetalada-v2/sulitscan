...
test("documents the implemented verification and article-only serving controls", () => {
  ...
  const cmpStep = readme.indexOf("Google-certified consent management platform")
  const servingStep = readme.indexOf("NEXT_PUBLIC_ADSENSE_ADS_ENABLED=true")
  assert.ok(cmpStep >= 0, "README must require an external Google-certified consent management platform")
  assert.ok(cmpStep < servingStep, "README must require the CMP before enabling AdSense serving")
  assert.match(readme, /EEA, United Kingdom, and Switzerland/)
