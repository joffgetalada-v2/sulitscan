...
- Non-object body, invalid or over-254-character email, an email whose local part contains `?`, `#`, or `/`, or `consent !== true`: HTTP 422. Common plus-addressing remains valid.
...
- Node tests cover validation (including literal path-delimiter and plus-address cases), consent, source normalization, two-operation existing/new success paths, preserved opt-out, new creation, concurrent create recovery, exact failure categories, sanitized route logging, SDK error objects, and thrown failures.
